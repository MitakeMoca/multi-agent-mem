...
def _sum_metrics(results: list[dict[str, Any]]) -> Metrics:
    ...
    for item in results:
        ...
        total.lifecycle_memory_hits += metrics.get("lifecycle_memory_hits", 0)
        total.linked_memory_hits += metrics.get("linked_memory_hits", 0)
...
def _compare(text_total: Metrics, structured_total: Metrics) -> dict[str, Any]:
    return {
        ...
        "lifecycle_memory_hits": structured_total.lifecycle_memory_hits,
        "linked_memory_hits": structured_total.linked_memory_hits,
    }
