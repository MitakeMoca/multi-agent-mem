...
class MultiAgentRuntime:
    ...
    def run_task(self, task: Task) -> dict[str, Any]:
        ...
        self._record_memory_metadata(metrics, initial_memories)
        ...
        self._record_memory_metadata(metrics, memories)
    ...
    def _record_memory_metadata(self, metrics: Metrics, memories: list[MemoryRecord]) -> None:
        for memory in memories:
            if memory.memory_type or memory.version or memory.status or memory.use_count or memory.last_hit_at:
                metrics.lifecycle_memory_hits += 1
            if memory.parent_memory_id or memory.linked_memory_ids or memory.link_type not in {"", "new_seed"}:
