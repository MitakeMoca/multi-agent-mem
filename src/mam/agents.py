...
class SummarizerAgent(BaseAgent):
    ...
    def summarize(
        ...
        record = memory.add(
            ...
            parent_memory_id=reused_ids[0] if reused_ids else "",
            linked_memory_ids=reused_ids,
            link_type="reuse_chain" if reused_ids else "new_seed",
            evolution_reason="复用历史协作结论生成新任务记忆" if reused_ids else "新任务组的初始协作记忆",
        )
        return {
