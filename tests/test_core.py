...
class CoreTest(unittest.TestCase):
    ...
    def test_memory_search_by_tag_and_semantic_text(self) -> None:
        ...
        self.assertEqual(hits[0].memory_type, 'procedural')
        self.assertGreaterEqual(hits[0].use_count, 1)
        self.assertTrue(hits[0].last_hit_at)

    def test_memory_lifecycle_and_link_metadata(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            memory = SharedMemory(Path(tmp) / 'linked.sqlite')
            parent = memory.add('summarizer', '协议记忆', '初始协议经验', ['protocol'], 'evidence', 'strategy')
            child = memory.add(
                'summarizer',
                '协议记忆复用',
                '复用协议经验生成新总结',
                ...
                linked_memory_ids=[parent.memory_id],
                link_type='reuse_chain',
                evolution_reason='测试动态链接字段',
            )
            hits = memory.search('协议 复用 总结', ['protocol'], top_k=3, min_score=0.0)
            memory.close()
        by_id = {record.memory_id: record for record in hits}
        self.assertEqual(child.parent_memory_id, parent.memory_id)
        self.assertIn(parent.memory_id, child.linked_memory_ids)
        self.assertIn(child.memory_id, by_id)
        self.assertEqual(by_id[child.memory_id].link_type, 'reuse_chain')
        self.assertGreaterEqual(by_id[child.memory_id].use_count, 1)
    ...
    def test_structured_runtime_runs_one_task(self) -> None:
        ...
        self.assertIn('lifecycle_memory_hits', result['metrics'])
        self.assertIn('linked_memory_hits', result['metrics'])
    ...
    def test_metrics_to_dict(self) -> None:
        ...
        self.assertIn('lifecycle_memory_hits', d)
        self.assertIn('linked_memory_hits', d)
